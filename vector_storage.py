...
class VectorStorage:
    ...
    def __init__(self, storage_dir: str = "vector_storage", embedding_dimension: int = 1024):
        """
    ...
    def cleanup_deleted_chunks(self):
        """
        Remove deleted chunks from metadata (FAISS index rebuild would be needed for full cleanup)
        """
        original_count = len(self.chunk_metadata)
        self.chunk_metadata = [m for m in self.chunk_metadata if not m.get("deleted", False)]
        cleaned_count = original_count - len(self.chunk_metadata)
