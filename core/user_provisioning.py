...
def has_identity(user_id: str) -> bool:
    """True when this user already has a stored name."""
    try:
    ...
    try:
        ...
        JournalStore(user_id=user_id).append(
            make_event(
                ...
                key="identity.name",
                value={"name": name},
                # Deliberately below the "user told me" bar: this came from a
                # platform profile, not from the human. If they later say
                # "actually call me X", that explicit fact supersedes this.
                confidence=0.6,
                source="inferred",
                extractor="deterministic",
                session_id="first_contact",
                turn_id=f"first_contact_{channel or 'channel'}",
                evidence={"note": f"{channel or 'channel'} profile display name"},
                applied=True,
            )
        )
        print(f"LOG: seeded first-contact profile for {user_id} ({channel}) name={name!r}")
        return True
